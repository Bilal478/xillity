    ...
    if($stmt->execute())
    {
        ...
        if ($result->num_rows > 0){
            while($row = $result->fetch_assoc()) 
            {
                ...
                $id = $row['id'];
                $passwordLink = $g_website."/update-password.php?id=$rand";
    ...
    { 
      ...
      if (!$g_enable2fa) {
        ...
        if ($_POST['remember_me']) {
            ...
            $sql = "UPDATE ".$g_projectSlug."_users SET remember_token='$token' WHERE email='$email'";
...
if(isset($_POST['create_user'])){
    ...
    $country = mb_htmlentities(($_POST['country']?$_POST['country']:''));
    ...
    if($actionId==""){
        ...
        if($actionId==""){
            $query = "insert into ".$g_projectSlug."_users set id='$id', name='$name', email='$email', phone='$phone', password='$password',
            businessName='$businessName', mobilePhone='$mobilePhone',country='$country',cnic_no='$cnic', homePhone='$homePhone', address='$address', city='$city', state='$state', zip='$zip', 
